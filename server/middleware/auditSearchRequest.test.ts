...
describe('auditSearchRequest', () => {
  ...
  it('should audit anonymously when user authentication is disabled', async () => {
    // Arrange
    const anonymousResponse = { locals: {} } as Response

    // Act
    await auditSearchRequest({ services, page: Page.SEARCH_OFFENDERS })(req, anonymousResponse, next)

    // Assert
    expect(logAuditEvent).toHaveBeenCalledWith(
      expect.objectContaining({ who: 'unknown', details: { build: 'abc1234', userRoles: [] } }),
    )
